...
// with the longCache header — rescans can rewrite cover bytes behind the
// same URL, so `immutable` would leave stale caches pinned (and is in
// any case banned project-wide; see server.go).
